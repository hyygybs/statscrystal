...
public class HeartCrystal extends Item {
    ...
    public InteractionResultHolder<ItemStack> use(Level world, Player player, InteractionHand hand) {
        ...
        if (!world.isClientSide) {
            player.getCapability(PlayerStatsProvider.PLAYER_STATS).ifPresent(capability -> {
                capability.addExtraHearts(1);
                capability.addExtraHealing(1);
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level world, List<Component> tooltip, TooltipFlag flag) {
        ...
        tooltip.add(Component.translatable("tooltip.statscrystal.heart_crystal").withStyle(ChatFormatting.GRAY,ChatFormatting.ITALIC));
    }
}
